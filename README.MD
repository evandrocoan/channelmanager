...
can open the command palette and type `StudioChannel: Generate Channel File`. See the following
...
1. **StudioChannel** This is one of the channels I implemented using the `ChannelManager`. I
   contains about 100 packages for the stable version and 150 for the development version. It has a
   custom installation wizard to guide you through the installation steps and call channel installer
   at the `ChannelManager` passing the channel configurations.

1. **AmxxChannel** This is a smaller channel of packages with about 20 packages focused on just for
   programming for AMX Mod X Pawn language.
...
the fix for build 3147 also broke completely the package for Sublime Text stable build 3143. Hence,
we must to create a tag which targets the last commit which is working for build 3143. Moreover when
some user using the stable build 3143 installs the `Notepad++ Color Scheme`, they must install the
one from the tag `3143`, and not the one from the master branch, which has the latest fixes for
build development build 3147 or newer.

Nonetheless, as currently we already fetch the lasted commit date version tag, it was easily
implemented automatic tagging for based on the latest commit date. It would be useful for tagging a
large set of packages which can be considered stable. Still also to do automatic tagging of the
latest release package is working for Sublime Text, however this seems not much required to be done
as should be only a few exceptions for when a packages stops working for some build of Sublime Text.
...
As well it would just be `dependency = 50`, then it would mean `Packages/Notepad++ Color Scheme` is
...
`.gitmodules` is compatible with `Python 3` because I created a wrapper which removes the
indentation before sending the files contents to the `configparser`:
...
plugin `submodules_manager.py`, with the command line with the argument `-f`. The
...
command to be passed by as a argument to allow select one specific repository to run the command,
...
1. Now you added the basics, you may notice we were not adding them as `git submodules` by the
   ...
   [submodule "Packages/MyBrandNewChannel"]
       ...
       upstream =
       dependency = ChannelManager
   ```

1. Now on this new package called `Packages/MyBrandNewChannel`, you need to create the plugin to do
   call to the channel manager passing its configurations. In the following we may see a full example
   configuration file, you can save it inside the folder `Packages/MyBrandNewChannel` with the name
   you like most as `settings.py` or `settings_and_commands.py`:
   ```python
   import os
...
1. Lastly we need to make these commands from our configuration file named
   ...
   [
       { "caption": "My Brand New Channel: Generate Channel File",
               "command": "my_brand_new_channel_generate_channel_file", "args": {"create_tags": false} },

       { "caption": "My Brand New Channel: Generate Channel File and Tags",
               "command": "my_brand_new_channel_generate_channel_file", "args": {"create_tags": true} },

       { "caption": "My Brand New Channel: Run Backstroke Requests",
               "command": "my_brand_new_channel_run", "args": {"run": "-b"} },

       { "caption": "My Brand New Channel: Pull & Rebase all Packages",
               "command": "my_brand_new_channel_run", "args": {"run": "-p"} },

       { "caption": "My Brand New Channel: Update Default Packages",
               "command": "my_brand_new_channel_update_default_packages" },

       { "caption": "My Brand New Channel: Run Stable Installation ",
               "command": "my_brand_new_channel_run_installation", "args": {"version": "stable"} },

       { "caption": "My Brand New Channel: Run Development Installation",
               "command": "my_brand_new_channel_run_installation", "args": {"version": "development"} },
...
### Multiple Channels Installation
...
Yet still not implemented on the `ChannelManager` dependency, an short installation wizard which
should allow you to choose which packages of Channel you do not want to install. It would also
present to you a list of packages which are currently installed and where pruned from the
installation due they being already installed on the system. Therefore you could select which of
these packages you would like to install over the packages you already has installed.
...
1. **YourChannelName: Generate Channel File and Tags** Similar to the last command, but it also
   create tags for each repository on based on the date of the last commit. The current tag, if it
   exists, if fetched and then its patch component (major.minor.patch) is incremented in one, if
   the latest
...
1. **YourChannelName: Pull & Rebase all Packages** Walk through all git repositories and perform the
...
1. **YourChannelName: Update Default Packages** Run the script `copy_default_package.py` which
...
1. Sublime Text my enter on deadlock, while Package Control is installing the missing dependencies.
   ...
   wich not CPU usage, i.e., for some time it is unresponsive, then you need to kill Sublime Text
   ...
   Sublime Text to restore the Sublime Text packages.
